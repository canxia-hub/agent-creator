...
- [ ] 🆕 是否包含 Gateway 重启安全规则（发出重启命令后禁止任何后续操作）?

#### 🆕 Gateway 重启安全规则（必须包含）

在 `AGENTS.md` 的 `safety_boundaries` 中，必须包含以下规则：

```xml
<safety_boundaries>
  <red_lines>
    <!-- ... 其他规则 ... -->
    <rule id="no-action-after-restart-command" priority="critical">
      gateway restart 命令发出后，禁止执行任何后续操作（包括但不限于：
      继续调用工具、发送消息、读写文件、启动子任务）。
      Gateway 重启会导致 event loop 阻塞、WebSocket 断连、会话锁超时，
      任何并发操作都会失败或产生竞态问题。
      正确做法：发出重启命令后立即停止，等待系统重启完成并触发 BOOT.md 启动检查。
    </rule>
  </red_lines>
</safety_boundaries>
```
...
Gateway 重启时会发生以下事件：
1. **Event Loop 阻塞**：重启过程中 Node.js event loop 会被阻塞，所有定时器、回调都无法执行
2. **WebSocket 断连**：客户端连接会断开，任何消息发送都会失败
3. **Session Lock 超时**：会话写入锁会因为 event loop 阻塞而无法及时释放，超过 max=15000ms 后被强制释放
4. **Main Session Recovery Failure**：transcript tail 因为重启中断而无法恢复
...
**正确行为**：发出 `openclaw gateway restart` 后，立即停止当前会话的所有操作，等待 Gateway 重启完成后由 BOOT.md 触发启动检查，再恢复工作。
